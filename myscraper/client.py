...
class Client(object):
    ...
    def _get_rating(self, product):
        ...
        rating = re.search(r'(\d.\d) out of 5 stars', str(product))
    ...
    def _get_prices(self, product):
        ...
        # get first a-price that contains main price
        main_price = product.find(attrs={"class":"a-price"})

        if main_price == None:
            return
        else:
            # match all prices of the form $X,XXX.XX:
            raw_prices = main_price.find_all(text=re.compile('\$[\d,]+.\d\d'))
        ...
        for raw_price in raw_prices:
            ...
            # convert string to float and replace European decimal seperator ',' with '.'s
            price_str = re.search('\$([\d,]+.\d\d)', raw_price).groups()[0]
    ...
    def _extract_page(self, page, max_product_nb):
        ...
        for product in products:
            ...
            print('Extracting {}'.format(product_dict['title'][:150]))
            ...
            product_dict['main_price'] = prices["prices_main"]

            # Add product html size to origin
            product_dict['html_size'] = len(str(product))

            if not product_dict['html_size']:
                print('  Failed to extract HTML_SIZE!')
